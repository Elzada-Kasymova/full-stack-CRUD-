...
function App() {
  ...
  const fetchClients = async () => {
        try {
          const response  = await axios.get('http://localhost:8383/api/clients')
  ...
  const handleSubmit = async (newClientData) => {
    if (modalMode === 'add') {
      try {
        const response = await axios.post('http://localhost:8383/api/clients', newClientData); // Replace with your actual API URL
      ...
      console.log('Updating client with ID:', clientData.id); // Log the ID being updated
            try {
                const response = await axios.put(`http://localhost:8383/api/clients/${clientData.id}`, newClientData);
